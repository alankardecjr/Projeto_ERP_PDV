...
class JanelaCadastroVendas(tk.Toplevel):
    ...
    def setup_sessao_cliente(self):
        ...
        tk.Label(f_busca, text="🔍 BUSCA RÁPIDA", font=("Segoe UI", 12, "bold"), bg=self.bg_fundo).pack(anchor="w")
        ...
        self.frame_dados_detalhados = tk.LabelFrame(frame_cli, text=" 👤 DADOS DO CLIENTE ", 
                                                    bg=self.bg_card, fg=self.cor_texto, font=("Segoe UI", 12, "bold"))
    ...
    def setup_sessao_produtos(self, parent):
        f_prod = tk.LabelFrame(parent, text=" 👠 LISTA DE PRODUTOS ", bg=self.bg_fundo, fg=self.cor_texto, font=("Segoe UI", 12, "bold"))
        ...
        cols = ("sku", "prod", "cor", "tam", "preco", "qtd")
    ...
    def setup_sessao_carrinho(self, parent):
        f_cart = tk.LabelFrame(parent, text=" 🛒 CARRINHO ", bg=self.bg_card, fg=self.cor_destaque, font=("Segoe UI", 12, "bold"))
        f_cart.pack(side="right", fill="both", expand=True)

        self.tree_cart = ttk.Treeview(f_cart, columns=("sku", "prod", "cor", "tam", "qtd", "sub"), show="headings", style="PDV.Treeview")
        for col in ("sku", "prod", "cor", "tam", "qtd", "sub"):
    ...
    def adicionar_ao_carrinho(self, event):
        ...
        if id_prod in self.carrinho_itens:
            ...
            self.carrinho_itens[id_prod] = {
                'id': id_prod, 'prod': item[1], 'cor': item[2], 'tam': item[3],
    ...
    def atualizar_view_carrinho(self):
        ...
        for dados in self.carrinho_itens.values():
            self.tree_cart.insert("", "end", values=(
                dados['sku'], dados['prod'], dados['cor'], dados['tam'], dados['qtd'], f"R$ {dados['sub']:.2f}"
            ))
