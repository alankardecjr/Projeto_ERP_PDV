...
class JanelaCadastroClientes(tk.Toplevel):
    ...
    def criar_widgets(self):
        ...
        def criar_campo(parent, texto, row, col=0, colspan=2, width=None):
            ...
            def on_focus_in(e):
                e.widget.config(highlightbackground=self.cor_destaque, highlightthickness=2)
        ...
        self.btn_salvar = tk.Button(btn_frame, text="SALVAR DADOS", bg=self.cor_btn_acao, fg="white", 
        ...
        self.btn_salvar.bind("<Leave>", lambda e: ao_sair_botao(e, self.cor_btn_acao))

        self.btn_venda = tk.Button(btn_frame, text="GERAR VENDA", bg=self.cor_btn_acao, fg="white", 
                                    font=("Segoe UI", 9, "bold"), width=15, relief="flat", cursor="hand2", 
                                    command=self.salvar_e_vender)
        self.btn_venda.pack(side="left", padx=5, ipady=6)
        self.btn_venda.bind("<Enter>", lambda e: ao_entrar_botao(e, self.cor_hover_btn))
        self.btn_venda.bind("<Leave>", lambda e: ao_sair_botao(e, self.cor_btn_acao))
        ...
        self.btn_cancelar.bind("<Enter>", lambda e: ao_entrar_botao(e, self.cor_hover_btn))
    ...
    def validar_e_salvar(self):
        ...
        if not d["nome"] or not d["tel"]:
            messagebox.showwarning("Atenção", "Os campos NOME e TELEFONE devem ser preenchidos.")
            return False
        try:
            if self.cliente_id:
